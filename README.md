...
Папка 'emotion_detector_working_build' содержит рабочую сборку программы. Запустить можно только на локальном компьютере так как библиотека cv2 с помошью которой реализовалась функция трансляции с камеры не работает на удаленных серверах типа 'google.colab'. В файле 'detector2' нужно указать пути до файлов 'emoji_array.npy' и 'model_old.h5'
