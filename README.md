...
Запустить тестирование работы обученной сети можно только на локальном компьютере так как библиотека cv2 с помошью которой реализовалась функция трансляции с камеры не работает на удаленных серверах типа 'google.colab'. Чтобы запустить сеть необходимо скачать файлы: 'detector.ipynb', 'emoji_array.npy' и 'model_old.h5', поместить их в одну папку и запустить файл 'detector.ipynb'. Чтобы запустить саму сеть нужно открыть ссылку и нажать 'ctrl+enter' https://colab.research.google.com/drive/1lbjhjEGzLq6rSYR3xYGRWkRmjB1UHaCO?usp=sharing
